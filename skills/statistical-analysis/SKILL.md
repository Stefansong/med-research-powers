...
按 `references/output-templates.md` 第 1 节模板生成（记录缺失、异常值、类型校正、重编码、清洗前后样本量对比）。
...
记录分析过程中的每一步决策和发现。按 `references/output-templates.md` 第 2 节模板生成（含 Pre-Analysis Checks、逐条分析执行、与 SAP 的偏差、敏感性分析、遇到的问题）。

#### 文件 3: `results-summary.md`（结果报告）

供 `manuscript-writing` 直接使用的结构化结果报告。按 `references/output-templates.md` 第 3 节模板生成（含样本特征、主要/次要结局、亚组、敏感性分析、Key Numbers for Abstract、所需图表清单）。
...
### 可选衔接
- 分析中发现需要调整方法 / 新增分析 → 可回到 `data-analysis-planning` 更新 SAP（并在 `analysis-log.md` 记录偏差）
- 结果需要进一步规范报告检查 → 可调用 `reporting-standards`（CONSORT/STROBE/TRIPOD 等）
