...
所有类型研究的方案设计——临床、基础、AI/ML、定性、问卷调查。先判断研究类型，再读取对应模块文件（`references/modules/` 下五个文件，见 Router 表），最终统一产出 `study-protocol.md`（文件内 `type:` 字段区分五类）。本文件只放路由、通用流程与通用规则；各类型的决策树、专属规则和专属 Common Mistakes / Convergence / Red Flags 都在模块文件里。
...
按 Router 判定 `type:`，向用户复述一句"我判断这是 [类型]，因为 [理由]"，然后读取 Router 表里对应的模块文件（`references/modules/clinical.md` / `basic-science.md` / `ai-ml.md` / `qualitative.md` / `survey-delphi.md` 之一）。模块内有该类型的决策树（细分设计）和专属规则，后续每一步都按模块要求补充细节。
...
按模块逻辑给出**有依据**的数字：power analysis（效应量来源、α、β、脱落率）/ 生物学重复数 / 数据划分方案 + 外部验证 / 信息饱和范围 / 调查公式。样本量脚本统一用 `${CLAUDE_PLUGIN_ROOT}/skills/statistical-analysis/scripts/power_analysis.py`。写不出依据 → 不要编一个数字，改写"需预实验 / 需文献效应量"并列为待办。
...
1. 按 `references/protocol-templates.md` 中对应 `type:` 的骨架生成 `study-protocol.md`，逐节填写，不留空模板字段（确实未知的写 `TBD: [需要什么]`）
...
骨架按类型取自 `references/protocol-templates.md` 的 A–E 部分。无论哪一类，文件都必须含：
...
下游读取方：`research-ethics`（伦理与注册章节）、`journal-selection`（研究概要）、`data-analysis-planning`（结局、变量、分析概要）、`data-collection-tools`（变量表、标注流程、Prompt 标准化章节）、`manuscript-writing`（Methods）。
...
| "样本量写个大概就行" | 没有依据的数字审稿人一眼看穿；写不出依据就写待办 |
...
1. 研究类型已判定并向用户说明理由，对应模块已读取
2. 研究对象、比较/对照、数据来源已具体可操作
3. 样本量/数据规模有写明来源的依据
4. 主要结局（或主要研究问题）唯一且有定义与评估时点
5. 偏倚控制措施已按类型写入
6. 报告规范已选定并映射到 protocol 章节
7. `study-protocol.md` 已生成，模块的 Convergence 条目全部满足
8. **Hard Checkpoint 已获用户明确确认**，`status: confirmed`，`.mrp-state.json` 已更新
...
报告格式复用 `using-med-research-powers` 的统一 Checkpoint 报告（生成的文件 / 关键决策 / 需要注意 / 建议下一步），"关键决策"里写：研究类型、研究对象概要、样本量与依据、主要结局定义、对照/比较、预计周期、是否需伦理审查与注册。**只追加下面的"锁定项"段**：
