...
description: Use when the user receives a revision decision (Major/Minor) and needs to plan their response strategically. Triggers on "怎么改"、"修改策略"、"大修"、"小修"、"revision plan"、"reviewer要求太多". Complements responding-to-reviewers by adding strategic prioritization.
---
...
- 仅需要写 response letter 的格式 → `responding-to-reviewers`
- 论文未投出 → 先完成投稿流程
- 模拟审稿 → `peer-review-simulation`

## Workflow: 4-Step Revision Planning
...
3. `cover-letter-writing` → 重写 Cover Letter（适配新期刊）
...
当以下条件满足时完成：
1. 所有审稿意见已分类和排序
2. 每条意见有明确的 action plan
3. 修改时间表已制定
4. Disagreeable items 有防御策略
...
### 强制衔接
- 修改计划完成 → `responding-to-reviewers`（写 Response Letter）
- 修改完成后 → `pre-submission-verification`（重新检查）
...
- 改投 → `cover-letter-writing`（重写 Cover Letter）
