...
- 结局只看总事件数（不按聚类、不按暴露拆分）。**不**逐个聚类列出结局：暴露在聚类层面变化时（如术式因术者而异），按术者/中心看结局就等于在看暴露与结局的关系。"很多聚类内结局全相同、混合模型可能不收敛"这类情况，在 SAP 里预先写好处理办法（如改用 GEE 或按患者汇总），不靠计划前查看来决定。
...
- **聚类数少时的校正**：聚类数少于约 40 个时，GEE 的三明治标准误偏小、I 类错误膨胀（Li & Redden 2015）→ 用偏差校正的方差，并用 t 检验（自由度 = 聚类数 − 参数个数），不用 z 检验——聚类少时即使做了偏差校正，z 检验仍不可靠。按聚类大小的变异系数（CV = 聚类大小的标准差 ÷ 均值，计划前可以看）预先选校正方法：CV < 0.6 用 Kauermann-Carroll（KC），CV ≥ 0.6 用 Fay-Graubard（FG）；Mancl-DeRouen（MD）配 t 检验在聚类数少于 20 时过于保守（Li & Redden 2015）。常用软件默认给的是 z 检验的 P 值，t 检验要自己算（见第 3 节）。线性混合模型用 REML 估计，自由度用 Kenward-Roger 或 Satterthwaite。中心只有 2–3 个时估计不了中心间方差，把中心作为固定效应（哑变量）或分层因素。
- **多层结构**：逐层写明怎么处理（如帧先按视频汇总；视频嵌套在术者内 → 术者随机截距）。
- **ICC 与设计效应**：设计效应 DE = 1 + (m − 1) × ICC 只在各聚类大小相同时成立（m 为聚类大小）；聚类大小不等时用 DE = 1 + ((CV² + 1) × m̄ − 1) × ICC（m̄ 为平均聚类大小，CV 为聚类大小的变异系数），CV < 0.23 时两者差别可以忽略（Eldridge 2006）。有效样本量 = 总观测数 ÷ DE；前瞻性研究按 DE 放大样本量（yaml `clustered_data`）。这两个公式针对在聚类层面变化的暴露（如按中心、按术者、按患者整体给的治疗）；暴露在聚类内变化时（如病灶层的结石位置），不能直接套用，套用可能高估所需样本量，要按具体设计用专门方法或模拟来估算。
...
| GEE 小样本校正 + t 检验 | KC：`v <- geesmv::GEE.var.kc(y ~ x, id = "patient_id", family = binomial, data = d, corstr = "exchangeable")$cov.beta`；FG：`geesmv::GEE.var.fg()`（参数相同）。两者只返回各系数的方差（数据须按 id 排序），系数取同样设定的 `gee::gee()` 拟合结果；t 检验手算：`t <- b / sqrt(v)`，`p <- 2 * pt(-abs(t), df = K - length(b))`（K 为聚类数）。`glmtoolbox::glmgee()` 后 `vcov(fit, type = "bias-corrected")` 是 MD，它的 `summary()` 仍按 z 检验给 P 值 | `GEE(...).fit(cov_type="bias_reduced")` 只有 MD，结果表仍是 z 检验；t 检验手算：`2 * scipy.stats.t.sf(abs(res.params / res.bse), df=K - len(res.params))`。要 KC/FG 建议用 R |
...
- 聚类数少仍用普通三明治标准误 + z 检验；或做了偏差校正，却仍用软件默认的 z 检验 P 值。
- 聚类大小差别很大，样本量仍按等大聚类的设计效应公式算。
...
- Eldridge SM, Ashby D, Kerry S. Sample size for cluster randomized trials: effect of coefficient of variation of cluster size and analysis method. *Int J Epidemiol*. 2006;35(5):1292-1300. doi:10.1093/ije/dyl129
