...
只看结构和质量，**不看**结局。倾向性评分的建模、匹配和平衡检查全程不需要结局，应该在看结局之前完成：
- 治疗组与对照组的人数和比例：对照远多于治疗组 → 适合 1:1 或 1:k 匹配；两组人数相近 → 加权通常更合适。
...
- **加权方案**：IPTW（是否用稳定化权重；极端权重的截断阈值预先定，常按权重分布的第 1 和第 99 百分位）（Austin & Stuart 2015），或重叠权重（自动压低 PS 接近 0 或 1 的人的权重）。
...
- **效应估计与方差**：匹配后用考虑配对的方差（按配对 ID 的簇稳健标准误）；加权后用稳健（三明治）方差或 bootstrap（Austin & Stuart 2015）；生存结局在匹配/加权后的 Cox 模型里同样用稳健方差。可以在匹配/加权后的结局模型中再调整主要协变量。
- **敏感性分析**：E-value，点估计和 CI 中靠近 1 的那一端都要算（VanderWeele 2017）；换一种 PS 方法（匹配 ↔ 加权）；换卡钳或截断阈值。
...
| 匹配/加权后的 Cox | `survival::coxph(Surv(t, e) ~ trt, data = md, weights = weights, robust = TRUE, cluster = subclass)` | `lifelines.CoxPHFitter().fit(df, "t", "e", weights_col="w", robust=True, cluster_col="pair")` |
...
- 匹配后仍按独立样本算标准误；加权后用普通回归的标准误（把权重当真实人数）——CI 过窄。
