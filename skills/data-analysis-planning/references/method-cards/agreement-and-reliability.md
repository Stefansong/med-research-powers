...
- 评分对象数（患者/图像/病灶/视频）、评分者人数、每位评分者评了多少对象；是否每个对象都由同一批评分者评分（全交叉），还是不同对象由不同评分者评分——决定用哪种 ICC、用 Cohen 还是 Fleiss kappa。
- 评分类型：二分类、无序多分类、有序等级（几级）、连续测量（单位、取值范围、有无"<0.1"这类截断值）、分割掩膜（体素间距、2D 还是 3D）。
- 每位评分者各自的类别分布：某一类别极少或极多时，kappa 会被压低（kappa 悖论）。
- 缺失评分：谁缺、缺多少、缺在哪些对象上。
...
- **指标按数据类型选**：2 名评分者 × 无序分类 → Cohen's kappa；有序分级 → 加权 kappa（线性或二次权重，预先选定）；≥3 名评分者 × 分类 → Fleiss' kappa；连续测量 → ICC + Bland-Altman；分割 → Dice、IoU、95% Hausdorff 距离（HD95），每例分别算再汇总。
...
- **分割指标规则**：两者都为空掩膜时怎么记；HD95 用毫米（按体素间距换算），不用像素；按病灶还是按病例汇总；小结构单独报告（Maier-Hein 2024）。
...
- 问卷/量表的信度研究：`cosmin`（COSMIN 2021，仓库索引有条目，无本地清单，需人工核对）。
