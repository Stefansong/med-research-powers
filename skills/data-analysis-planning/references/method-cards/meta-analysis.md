...
- **诊断试验 Meta**：用双变量随机效应模型（Reitsma 2005；无协变量时与 HSROC 等价）同时合并灵敏度和特异度；报告 SROC 曲线及 95% 置信区域和预测区域；各研究阈值不同时以 SROC/HSROC 为主。
- **偏倚风险**：按设计选工具——RCT 用 RoB 2，非随机干预研究用 ROBINS-I，诊断试验用 QUADAS-2，预测模型用 PROBAST；两名评价者独立评价后核对。
...
| 诊断试验 Meta | `mada::reitsma(d)`（列名 TP、FN、FP、TN；`plot()` 画 SROC） | 无公认成熟包，建议用 R |
...
`meta::metabias()` 默认研究数 < 10 时不做检验，且对 OR 默认用 Harbord 检验、对诊断比值比用 Deeks 检验——不要为了出结果把 `k.min` 调小。
...
- 偏倚风险工具：`rob-2`、`robins-i`、`quadas-2`、`probast`；证据确定性：`grade`；综述方法学质量自查：`amstar-2`。
