...
> 适用：描述研究人群的基线表（Table 1）；两组或多组之间连续、分类、有序变量的比较及效应量；配对数据（同一批人前后两次、配对样本）的比较。　不适用：需要调整混杂才能回答的因果问题（见 `regression-and-prediction-models.md`、`propensity-score.md`）；生存结局（见 `survival-analysis.md`）；同一患者多条记录（见 `clustered-and-repeated-data.md`）。检验方法的选择分支以 `stat-method-decision-tree.yaml` 为准，本卡不重复决策树，只写要点与坑。
...
- **每个比较的方法与效应量**（分支按 yaml `two_groups` / `multiple_groups`；前提检验用 `assumption_tests.py`）：
  - 连续、近似正态：均数差 + 95% CI；需要跨研究比较时加 Hedges g（小样本校正后的 SMD）+ CI。
  - 连续、非正态：Hodges-Lehmann 位置差（两组间所有两两差值的中位数）+ CI，或中位数差 + bootstrap CI；只报 Mann-Whitney 的 p 不够。
...
两种语言默认值不同：R 的 `t.test` 默认 Welch，`scipy.stats.ttest_ind` 默认 `equal_var=True`（Student t）；计划里写明用哪一个。
